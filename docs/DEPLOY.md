...
- `APP_PASSWORD` — пароль входа в панель (если порт доступен из интернета)
- `SESSION_SECRET` — случайная строка 32+ символов (сессия входа; можно = `ENCRYPTION_KEY`)
...
Откроется страница **входа** (`/login`). YouTube OAuth callback (`/api/youtube/callback`) остаётся доступным без пароля — Google должен достучаться до него при авторизации.

Приложение открывается с вашего компьютера; при пробросе `:3000` в интернет обязательно задайте `APP_PASSWORD`.
